...
namespace Services
{
    ...
    {
        private const string key = "91e0d4296bdfc99f07241e1b39b1f41f";
        public const string TmbdHttpClientName = "TmdbClient";
        private readonly IHttpClientFactory _httpClientFactory;
        public TmdbService(IHttpClientFactory httpClientFactory)
        {
            _httpClientFactory = httpClientFactory;
        }
        private HttpClient HttpClient =>_httpClientFactory.CreateClient(TmbdHttpClientName);
        ...
        {
            var videosWrapper = await HttpClient.GetFromJsonAsync<VideosWrapper>($"{TmdbUrls.GetTrailers(id, type)}&api_key={key}");
